import {  HubbleImagesResponse, , News, , NewsResponse } from '@/utils/types'
...
const CardsGrid = ({objects , mode} : {objects: NewsResponse | HubbleImagesResponse , mode:string}): ReactNode => {
 ...
 if (mode==="hubble-page"){
  return <div className="grid gap-2 auto-rows-fr grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">{objects.results.map((item , index) => (
    <HubbleCard/>
  ))}
